...
class Database {
    ...
    public function buildWhere($fields = '', $ops=''){
        ...
        for ($f = 0; $f < count($fields); $f++){
            // replace any dots ('.') with underscores ('_') in the field list
            $bindHolders = str_replace( ".", "_", $fields[$f]);
            $op = $f < count($ops) ? $ops[$f] : $ops[0];
            $comparison = trim($op) . " :w_" . trim($bindHolders);
    ...
    public function buildSql(sqlParams $params){
        ...
        if (strtolower($params->type) == 'insert') {
            ...
            // replace any dots ('.') with underscores ('_') in the field list
            $bindHolders = str_replace( ".", "_", $bindHolders);
        ...
        if (strtolower($params->type) == 'update'){
            ...
            foreach ($fields as $f){
                // replace any dots ('.') with underscores ('_') in the field list
                $bindHolders = str_replace( ".", "_", $f);
                $fieldValues .= (strlen($fieldValues) > 0 ? ", " : "" ) .
                    "$f = :$bindHolders";
            }
            return "UPDATE $table SET $fieldValues$filters$limit";
        }
        $fieldList = ensureArray($fields);
        $fieldString = '';
        foreach ($fieldList as $f) {
            //$as = str_replace('.', '_', $f);
            $fieldString .= (strlen($fieldString) > 0 ? ', ' : '') .
                    "$f as '$f'";
        }
        if ($fields == ''){
            $fieldString = '*';
        }
        $sql = trim("SELECT $fieldString FROM $table$filters$sort$limit");
        return $sql;
    }
    ...
    public function fetchRecords($table = '', $fields = '', $filterFields = '',
        ...
        for ($f = 0; $f < count($fFields); $f++){
            if ($f < count($fValues)){
                $bindFields = "w_" . str_replace('.', '_', $fFields[$f]);
                $this->bind($bindFields, $fValues[$f]);
            }
        }
    ...
    public function addRecords($table = '', $fields = '', $values = ''){
        ...
        foreach ($fieldsList as $key) {
            ...
            $bindFields = str_replace('.', '_', $key);
            $this->bind($bindFields, $values[$key]);
        }
